...
mod tests {
    ...
    #[test]
    fn ace_beats_king() {
        assert!(card_value(&Card::new(Rank::Ace, Suit::Spades)) > card_value(&Card::new(Rank::King, Suit::Hearts)));
    }

    #[test]
    fn same_rank_is_tie() {
        assert_eq!(
            card_value(&Card::new(Rank::Seven, Suit::Spades)),
            card_value(&Card::new(Rank::Seven, Suit::Hearts))
        );
    }
    ...
    fn payout_win() {
        ...
        game.bet = 100;
        game.player_card = Some(Card::new(Rank::King, Suit::Spades));
        game.dealer_card = Some(Card::new(Rank::Ten, Suit::Hearts));
        game.resolve_reveal();
    ...
    fn payout_loss() {
        let mut game = WarGame::new();
        game.bet = 100;
        game.player_card = Some(Card::new(Rank::Five, Suit::Spades));
        game.dealer_card = Some(Card::new(Rank::Ten, Suit::Hearts));
        game.resolve_reveal();
        assert_eq!(game.last_payout, -100);
    }
