...
mod tests {
    ...
    #[test]
    fn face_cards_worth_zero() {
        assert_eq!(card_points(&Card::new(Rank::King, Suit::Spades)), 0);
        assert_eq!(card_points(&Card::new(Rank::Queen, Suit::Hearts)), 0);
        assert_eq!(card_points(&Card::new(Rank::Ten, Suit::Diamonds)), 0);
    }

    #[test]
    fn hand_total_wraps_at_10() {
        let hand = vec![Card::new(Rank::Seven, Suit::Spades), Card::new(Rank::Eight, Suit::Hearts)];
        assert_eq!(hand_total(&hand), 5); // 15 % 10 = 5
    }

    #[test]
    fn natural_nine() {
        let hand = vec![Card::new(Rank::Four, Suit::Spades), Card::new(Rank::Five, Suit::Hearts)];
        assert_eq!(hand_total(&hand), 9);
    }

    #[test]
    fn player_bet_win() {
        let mut game = BaccaratGame::new();
        game.bet = 100;
        game.bet_type = BaccaratBet::Player;
        game.player_hand = vec![Card::new(Rank::Nine, Suit::Spades), Card::new(Rank::Eight, Suit::Hearts)]; // 7
        game.banker_hand = vec![Card::new(Rank::Three, Suit::Spades), Card::new(Rank::Two, Suit::Hearts)]; // 5
        game.resolve();
        assert_eq!(game.last_payout, 100);
    }

    #[test]
    fn tie_bet_pays_8_to_1() {
        let mut game = BaccaratGame::new();
        game.bet = 100;
        game.bet_type = BaccaratBet::Tie;
        game.player_hand = vec![Card::new(Rank::Five, Suit::Spades), Card::new(Rank::Three, Suit::Hearts)];
        game.banker_hand = vec![Card::new(Rank::Six, Suit::Spades), Card::new(Rank::Two, Suit::Hearts)];
        game.resolve();
        assert_eq!(game.last_payout, 800);
    }
}
